...
function renderAssignmentBox(req) {
  ...
  if (canAccept) {
    if (alreadyAssigned) {
      actionButton = `<button class="btn btn--rinuncia" type="button" data-unaccept-assignment="${req.id}">Rinuncia incarico</button>`;
    } else {
    ...
    <div class="step step--compact">
      <div class="step__top">
        ...
        <div class="step__actions">
          <button class="btn btn--3" style="padding: 6px 10px; font-size: 12px;"
 ...
 * Login DEMO
 */
bindById('btnDemoLogin', 'click', function() {
  const nameInput = byId('demoName');
  const roleSelect = byId('demoRole');
 ...
 * Logout
 */
bindById('btnLogout', 'click', function() {
  if (confirm('Disconnettere?')) {
    clearSession();
    state.selectedRequestId = null;
    render();
    byId('modalConsuntivo')?.close();
    byId('modalRequest')?.close();
  }
});
 ...
 */
bindById('btnNewRequest', 'click', function() {
  byId('modalRequest')?.showModal();
});

/**
 * Chiudi modal nuova richiesta
 */
bindAll('[data-close-request]', 'click', function() {
  byId('modalRequest')?.close();
});

bindById('btnCancelReq', 'click', function() {
  byId('modalRequest')?.close();
});
 ...
 * Salva richiesta
 */
bindById('requestForm', 'submit', function(e) {
  e.preventDefault();
 ...
 * Ricerca richieste
 */
bindById('searchInput', 'input', function() {
  renderRequestList();
});
 ...
 * Chiudi consuntivo
 */
bindAll('[data-close-cons]', 'click', function() {
  closeConsuntivo();
});
 ...
 * Apri richiesta (consuntivo)
 */
bindById('btnDeleteReq', 'click', function() {
 ...
 */
bindById('chkRitirato', 'change', function() {
...
bindById('chkSaldato', 'change', function() {
  if (state.selectedRequestId) {
 ...
 * Aggiungi step
 */
bindById('btnAddStep', 'click', function() {
  if (state.selectedRequestId) {
 ...
 */
const reqNotesEl = byId('reqNotes');
if (reqNotesEl) {
...
function updateTransportCalculations() {
  const transportSCU = byId('transportSCU');
  const transportPrice = byId('transportPrice');
  const transportSpesaTotal = byId('transportSpesaTotal');
  const transportCompenso = byId('transportCompenso');
  const transportImportoTotal = byId('transportImportoTotal');
  if (!transportSCU || !transportPrice || !transportSpesaTotal || !transportCompenso || !transportImportoTotal) {
    return;
  }

  const scu = parseFloat(transportSCU.value) || 0;
  const price = parseFloat(transportPrice.value) || 0;
  
  const spesaTotal = scu * price;
  const compenso = spesaTotal * 0.10;
  const importoTotal = spesaTotal + compenso;
  
  transportSpesaTotal.textContent = spesaTotal.toFixed(2) + ' UEC';
  transportCompenso.textContent = compenso.toFixed(2) + ' UEC';
  transportImportoTotal.textContent = importoTotal.toFixed(2) + ' UEC';
}

bindById('transportSCU', 'input', updateTransportCalculations);
bindById('transportPrice', 'input', updateTransportCalculations);

/**
 * Modal trasporto: chiudi
 */
bindAll('[data-close-transport]', 'click', function() {
  const modal = byId('modalTransporto');
  if (!modal) return;
 ...
 */
bindById('transportForm', 'submit', function(e) {
  e.preventDefault();
